...
public class MapController extends centralController implements Initializable{
    ...
    public void initialize(URL fxmlFileLocation, ResourceBundle resources){
        initializeChoiceBox();
        mapPane.setBackground(new Background(new BackgroundFill(javafx.scene.paint.Color.BLACK,new CornerRadii(0),new Insets(0,0,0,0))));
        initializeMapImage();
//        mapImage.setViewport(new Rectangle2D(300,300,400,400));
        ...
        floorChoiceBox.getSelectionModel().selectedIndexProperty().addListener(
                new ChangeListener<Number>() {
                    public void changed(ObservableValue ov, Number old_value, Number new_value) {
                        System.out.println("You picked index: " + new_value + ". The old index was: " + old_value +". The value at that index is: " + floorChoiceBox.getItems().get((int)new_value));
                        System.out.println("Changing floor to: " + floorChoiceBox.getItems().get((int) new_value));
                        // Change the image that's being displayed when the input changes
                        Image new_img = new Image("/images/Floor Plans/" + (floorChoiceBox.getItems().get((int) new_value)) + "floor.png");
    ...
    public void mapScrolled(ScrollEvent e){
        ...
        double proportionX = (e.getSceneX() - mapImage.localToScene(mapImage.getBoundsInLocal()).getMinX())/mapImage.getFitWidth();
        double proportionY = (e.getSceneY() - mapImage.localToScene(mapImage.getBoundsInLocal()).getMinY())/mapImage.getFitHeight();
